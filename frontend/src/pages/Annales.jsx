...
/* ─── PDF pages (canvas-rendered, scrolls & zooms natively on mobile) ──────────── */
function PdfPages({ blobUrl }) {
  ...
  const pageWidth = containerWidth ? Math.min(containerWidth - 24, 900) * zoom : undefined;

  return (
    <div>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:10, padding:'8px 12px', background:C.bg, borderBottom:`1px solid ${C.border}` }}>
        <button onClick={() => setZoom(z => Math.max(0.5, +(z - 0.25).toFixed(2)))}
          style={{ width:28, height:28, borderRadius:8, border:`1.5px solid ${C.border}`, background:C.card, color:C.text, fontSize:16, fontWeight:700, cursor:'pointer', lineHeight:1 }}>
          −
        </button>
        <span style={{ fontSize:12, fontWeight:600, color:C.sub, minWidth:38, textAlign:'center' }}>{Math.round(zoom * 100)}%</span>
      ...
      <div ref={containerRef}
        style={{ width:'100%', maxHeight:'65vh', overflow:'auto', background:'#e2e8f0', WebkitOverflowScrolling:'touch' }}>
        {pdfError ? (
          <div style={{ textAlign:'center', padding:'32px 0', color:'#f87171', fontSize:13 }}>Impossible d'afficher ce PDF.</div>
        ) : (
          <Document
            file={blobUrl}
            ...
            loading={
              <div style={{ display:'flex', alignItems:'center', justifyContent:'center', padding:'40px 0' }}>
                <div style={{ width:24, height:24, border:`3px solid ${C.border}`, borderTopColor:C.indigo, borderRadius:'50%', animation:'spin 0.8s linear infinite' }}/>
              </div>
            }
          >
            {containerWidth > 0 && Array.from({ length: numPages }, (_, i) => (
              <div key={i} style={{ display:'flex', justifyContent:'center', padding:'12px 0' }}>

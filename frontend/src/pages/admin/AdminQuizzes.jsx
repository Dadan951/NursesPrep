...
export default function AdminQuizzes() {
  ...
  const load = () => { setLoading(true); axios.get(`${API_URL}/quizzes/admin`).then(r=>setQuizzes(r.data)).finally(()=>setLoading(false)); };
  useEffect(load, []);
  ...
  const filtered  = quizzes.filter(q => q.title.toLowerCase().includes(search.toLowerCase()) || (q.category||'').toLowerCase().includes(search.toLowerCase()));
        ...
        <div className="px-6 pb-8">
          <div className="bg-white rounded-3xl shadow-2xl overflow-hidden">
            ...
            {loading ? (
              ...
              <div className="text-center py-16 text-slate-400"><div className="text-5xl mb-3">🧠</div><p className="font-semibold">Aucun quiz trouvé</p></div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200">
                      {['Titre','Catégorie','Difficulté','Questions','Durée','Statut',''].map(h=>(
                  ...
                  <tbody>
                    {filtered.map((q,i)=>(
                      <motion.tr key={q._id} initial={{opacity:0,y:6}} animate={{opacity:1,y:0}} transition={{delay:i*0.025}}
                        className="border-b border-slate-100 hover:bg-blue-50/30 transition-all group">
                        <td className="px-5 py-3.5 text-sm font-semibold text-slate-800 max-w-xs truncate">{q.title}</td>
                        <td className="px-5 py-3.5"><span className="text-xs bg-blue-100 text-blue-700 px-2.5 py-1 rounded-full font-medium">{q.category}</span></td>
                        <td className="px-5 py-3.5"><span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${diffColors[q.difficulty]}`}>{diffLabel[q.difficulty]}</span></td>
                        <td className="px-5 py-3.5 text-xs font-semibold text-slate-700">{q.questions?.length||0}</td>
                        <td className="px-5 py-3.5 text-xs text-slate-500">{q.duration} min</td>
                        <td className="px-5 py-3.5"><span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${q.isPublished?'bg-emerald-100 text-emerald-700':'bg-slate-100 text-slate-500'}`}>{q.isPublished?'Publié':'Brouillon'}</span></td>
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-1.5 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={()=>setModal(q)} className="p-1.5 rounded-lg text-slate-400 hover:text-blue-600 hover:bg-blue-50 transition">
                              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                            </button>
                            <button onClick={()=>setDeleting(q)} className="p-1.5 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50 transition">
